...
class WholeSaleRequisition(models.Model):
    ...
    def _constrains_requisition(self):
        for line in self:
            for req in line.requisition_line_ids:
                if not req.multi_uom_line_id or not req.required_qty > 0.0:
                    raise ValidationError("You must add uom (or) required qty to the requisition")
    ...
    def action_confirm(self):
        for rec in self.requisition_line_ids:
            if rec.product_id and rec.required_qty <= 0:
    ...
    def action_approved(self):
        for rec in self.requisition_line_ids:
            if rec.product_id and rec.allowed_qty <= 0:
...
class WholeSaleRequisitionLine(models.Model):
    ...
    required_qty = fields.Float(string='Required Qty', default=0.0, required=True)
