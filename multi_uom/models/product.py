...
class MultiUOMLine(models.Model):
    ...
    uom_code = fields.Char(string='UOM Code', compute='compute_uom_code', store=True)

    @api.depends('product_tmpl_id', 'uom_id')
    def compute_uom_code(self):
        for rec in self:
            if rec.uom_id and rec.product_tmpl_id and rec.product_tmpl_id.default_code:
                rec.uom_code = '[' + rec.product_tmpl_id.default_code + ']' + rec.uom_id.name
            else:
                rec.uom_code = False
