...
class PurchaseRequisitionLine(models.Model):
    ...
    multi_allowed_qty = fields.Float(string='Multi Allowed Quantity', digits='Product Unit of Measure', copy=False)
    ...
    def action_purchase_agreement(self):
        ...
        for line in lines:
            if line.purchase_requisitions_id.id not in pr_lines:
                pr_lines.append(line.purchase_requisitions_id.id)
        purchase_agreement = self.env['purchase.requisition'].create({
            'ordering_date': Date.today(),
            'purchase_stock_requistion_ids': pr_lines
        })
        is_created = False
        for key, grouped_lines in groupby(lines, lambda l: (l.product_id.id, l.multi_uom_line_id.id)):
            ...
            agreement_lines.append({
                ...
                'purchase_stock_requisitions_id': first_line.purchase_requisitions_id.id
            })
