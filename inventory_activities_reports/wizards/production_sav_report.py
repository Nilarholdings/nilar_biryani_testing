...
class ProductionSAVReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet.merge_range('A1:K1', 'Production Standard, Actual and Variance Report', title)
        ...
        for record in records:
            ...
            sheet.write(f'D{row_index}', record.multi_uom_line_id.uom_id.name or '', cell_center)
            sheet.write(f'E{row_index}', record.standard_quantity or 0.0, cell_center)
            sheet.write(f'F{row_index}', record.multi_uom_line_id.uom_id.name or '', cell_center)
            sheet.write(f'G{row_index}', record.multi_qty_producing or 0.0, cell_center)
            sheet.write(f'H{row_index}', record.multi_uom_line_id.uom_id.name or '', cell_center)
            sheet.write(f'I{row_index}', record.difference_quantity or 0.0, cell_center)
            sheet.write(f'J{row_index}', record.multi_uom_line_id.uom_id.name or '', cell_center)
            row_index += 1
            ...
            for move in record.move_raw_ids:
                ...
                sheet.write(f'D{row_index}', move.multi_uom_line_id.uom_id.name or '', cell_center)
                sheet.write(f'E{row_index}', move.standard_qty or 0.0, cell_center)
                sheet.write(f'F{row_index}', move.multi_uom_line_id.uom_id.name or '', cell_center)
                sheet.write(f'G{row_index}', move.multi_quantity_done or 0.0, cell_center)
                sheet.write(f'H{row_index}', move.multi_uom_line_id.uom_id.name or '', cell_center)
                sheet.write(f'I{row_index}', move.difference_qty or 0.0, cell_center)
                sheet.write(f'J{row_index}', move.multi_uom_line_id.uom_id.name or '', cell_center)
                row_index += 1
