...
SELECT      {0},
            ...
            SUM(UNBUILD_QTY) AS UNBUILD_QTY,
            SUM(INTERNAL_TRANSFER_QTY) AS INTERNAL_TRANSFER_QTY,
            SUM(GOOD_RECEIVED_QTY) AS GOOD_RECEIVED_QTY,
            SUM(ADJUSTMENT_QTY) AS ADJUSTMENT_QTY,
            SUM(SCRAP_QTY) AS SCRAP_QTY,
            SUM(RECEIVED_QTY) AS RECEIVED_QTY,
            SUM(DELIVER_QTY) AS DELIVER_QTY,
            SUM(POS_QTY) AS POS_QTY,
            SUM(POS_RETURN_QTY) AS POS_RETURN_QTY
FROM
(
SELECT      SML.PRODUCT_ID,
            ...
            CASE
                WHEN SM.PURCHASE_LINE_ID IS NULL AND SL.USAGE='transit' AND DL.USAGE='internal' AND DL.ID IN {1}
                THEN SML.QTY_DONE
                WHEN SM.PURCHASE_LINE_ID IS NULL AND DL.USAGE='transit' AND SL.USAGE='internal' AND SL.ID IN {1}
                THEN -SML.QTY_DONE
                ELSE 0
            ...
            END AS SCRAP_QTY,
            CASE
                WHEN SM.PURCHASE_LINE_ID IS NULL AND SL.USAGE='supplier' AND DL.USAGE='internal' AND DL.ID IN {1}
                THEN SML.QTY_DONE
                WHEN SM.PURCHASE_LINE_ID IS NULL AND SL.USAGE='internal' AND DL.USAGE='supplier' AND SL.ID IN {2}
                THEN -SML.QTY_DONE
                ELSE 0
            END AS RECEIVED_QTY,
            CASE
                WHEN SM.SALE_LINE_ID IS NULL AND DL.USAGE='customer' AND SL.ID IN {3}
                THEN -SML.QTY_DONE
                WHEN SM.SALE_LINE_ID IS NULL AND DL.USAGE='internal' AND DL.ID IN {4}
                THEN SML.QTY_DONE
                ELSE 0
            END AS DELIVER_QTY,
            
            CASE
                WHEN SL.USAGE='internal' AND DL.USAGE='customer' AND SL.ID IN {10} AND SM.SALE_LINE_ID IS NULL 
                THEN -SML.QTY_DONE
                ELSE 0
            END AS POS_QTY,
            CASE
                WHEN SL.USAGE='customer' AND DL.USAGE='internal' AND DL.ID IN {11} AND SM.SALE_LINE_ID IS NULL 
                THEN SML.QTY_DONE
                ELSE 0
...
SELECT      SML.PRODUCT_ID,
            ...
            0 AS PRODUCTION_QTY,
            0 AS UNBUILD_QTY,
            SML.QTY_DONE AS INTERNAL_TRANSFER_QTY,
            0 AS GOOD_RECEIVED_QTY,
            0 AS ADJUSTMENT_QTY,
            0 AS SCRAP_QTY,
            0 AS RECEIVED_QTY,
            0 AS DELIVER_QTY,
...
class StockInOutReport(models.AbstractModel):
    ...
    def _get_opening(self, location, product, date_from):
        query = """
        SELECT			
                        SUM(QTY) AS QTY
        FROM
        (
            SELECT		
                        -QTY_DONE AS QTY 
            FROM		STOCK_MOVE_LINE SML
            WHERE		SML.STATE= 'done' AND SML.DATE < '{1}' AND SML.LOCATION_ID = {0} AND SML.PRODUCT_ID = {2}
            
            UNION ALL	
            SELECT		
                        QTY_DONE AS QTY
            FROM		STOCK_MOVE_LINE SML
            WHERE		SML.STATE= 'done' AND SML.DATE < '{1}' AND SML.LOCATION_DEST_ID = {0} AND SML.PRODUCT_ID = {2}
        ) OPENING
        """.format(location, date_from, product)
        self.env.cr.execute(query)
        records = self.env.cr.dictfetchall()
        opening_qty = 0
        for record in records:
            opening_qty += record['qty'] or 0
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet.merge_range('C3:D3', self.env.company.name, header_cell_left)
        sheet.merge_range('A4:B4', 'Location', header_cell_left)
        sheet.merge_range('C4:D4', ', '.join(location_ids.mapped('complete_name')), header_cell_left)
        sheet.merge_range('A5:B5', 'Beginning Date', header_cell_left)
        sheet.merge_range('C5:D5', start_date_raw.strftime('%d %B, %Y'), header_cell_left)
        sheet.merge_range('A6:B6', 'Ending Date', header_cell_left)
        sheet.merge_range('C6:D6', end_date_raw.strftime('%d %B, %Y'), header_cell_left)
        ...
        sheet.set_column('AG:AH', 10)
        sheet.set_column('AI:AJ', 10)
        ...
        sheet.merge_range(f'Y9:Z9', 'Internal Transfer/Sale Requisition', header_cell)
        sheet.merge_range(f'AA9:AB9', 'Received', header_cell)
        sheet.merge_range(f'AC9:AD9', 'Stock Requisition', header_cell)
        sheet.merge_range(f'AE9:AF9', 'Scrap', header_cell)
        sheet.merge_range(f'AG9:AH9', 'Repackaging', header_cell)
        sheet.merge_range(f'AI9:AJ9', 'Unpackaging', header_cell)
        sheet.merge_range(f'AK9:AL9', 'Closing', header_cell)
        ...
        for col_group in columns:
            sheet.set_column(f'{col_group[0]}:{col_group[0]}', 12)
            sheet.set_column(f'{col_group[1]}:{col_group[1]}', 18)
    ...
    def group_by_products(self, workbook, sheet, grouped_records, start_date_raw, end_date_raw, allowed_locations,
        ...
        total_opening_unpackaging_quantity = total_opening_repackaging_quantity = 0
        for location, records in grouped_records:
            ...
            total_production_qty = total_unbuild_qty = total_unpackaging_qty = total_repackaging_qty = total_good_received_qty = 0
            total_purchase_qty = total_purchase_return_qty = total_sale_qty = total_sale_return_qty = total_pos_qty = total_pos_return_qty = 0
            total_internal_transfer_qty = total_adjustment_qty = total_scrap_qty = total_opening_qty = total_closing_qty = total_received_qty = total_deliver_qty =0
            ...
            for record in records:
                ...
                if no_transaction:
                    opening_qty = self._get_opening(location.id, product.id, start_date_raw)
                else:
                    opening_qty = self._get_opening(location.id, product.id, start_date_raw)
                ...
                opening_unpackaging_product = self.env['stock.repackaging.line'].search(
                    [('product_id', '=', product.id), ('repackaging_id.state', '=', 'done'),
                     ('repackaging_id.date', '<', start_date_raw),
                     ('repackaging_id.location_id', '=', location.id)])

                opening_repackaging_product = self.env['stock.packaging.line'].search(
                    [('product_id', '=', product.id), ('stock_package_id.state', '=', 'done'),
                     ('stock_package_id.date', '<', start_date_raw),
                     ('stock_package_id.location_id', '=', location.id)])

                unpackaging_quantity = repackaging_quantity = 0
                opening_unpackaging_quantity = opening_repackaging_quantity = 0
                if unpackaging_product:
                    ...
                    opening_unpackaging_quantity = sum(
                        opening_unpackaging_product.filtered(lambda m: m.product_id.id == product.id).mapped(
                            "quantity"))
                if repackaging_product:
                    ...
                    opening_repackaging_quantity = sum(
                        opening_repackaging_product.filtered(lambda m: m.product_id.id == product.id).mapped("qty"))
                total_opening_repackaging_quantity += opening_repackaging_quantity
                total_opening_unpackaging_quantity += opening_unpackaging_quantity

                opening_qty = opening_qty + opening_repackaging_quantity + opening_unpackaging_quantity
                ...
                good_received_qty = record['good_received_qty']
                ...
                deliver_qty = record['deliver_qty']
                closing_qty = opening_qty + received_qty + purchase_qty + purchase_return_qty + sale_qty + sale_return_qty + pos_qty + pos_return_qty + production_qty + unbuild_qty + good_received_qty + adjustment_qty + scrap_qty + repackaging_quantity + unpackaging_quantity + internal_transfer_qty
                if any([opening_qty, received_qty, closing_qty, purchase_qty, purchase_return_qty, sale_qty, sale_return_qty, pos_qty,
                        pos_return_qty, production_qty, unbuild_qty, good_received_qty, adjustment_qty, scrap_qty,
                        repackaging_quantity, unpackaging_quantity, internal_transfer_qty]):
                    ...
                    sheet.write(f'L{row_index}', product.product_tmpl_id.convert_to_multi_uom(sale_qty),
                                normal_center_cell)
                    ...
                    sheet.write(f'P{row_index}', product.product_tmpl_id.convert_to_multi_uom(pos_qty),
                                normal_center_cell)
                    ...
                    sheet.write(f'AA{row_index}', received_qty, normal_center_cell)
                    sheet.write(f'AB{row_index}', product.product_tmpl_id.convert_to_multi_uom(received_qty),
                                normal_center_cell)



                    sheet.write(f'AC{row_index}', good_received_qty, normal_center_cell)
                    sheet.write(f'AD{row_index}', product.product_tmpl_id.convert_to_multi_uom(good_received_qty),
                                normal_center_cell)

                    sheet.write(f'AE{row_index}', scrap_qty, normal_center_cell)
                    sheet.write(f'AF{row_index}', product.product_tmpl_id.convert_to_multi_uom(scrap_qty),
                                normal_center_cell)
                    sheet.write(f'AG{row_index}', repackaging_quantity, normal_center_cell)
                    sheet.write(f'AH{row_index}', product.product_tmpl_id.convert_to_multi_uom(repackaging_quantity),
                                normal_center_cell)
                    sheet.write(f'AI{row_index}', unpackaging_quantity, normal_center_cell)
                    sheet.write(f'AJ{row_index}', product.product_tmpl_id.convert_to_multi_uom(unpackaging_quantity),
                                normal_center_cell)
                    sheet.write(f'AK{row_index}', closing_qty, normal_center_cell)
                    sheet.write(f'AL{row_index}', product.product_tmpl_id.convert_to_multi_uom(closing_qty),
                                normal_center_cell)
            ...
            sheet.write(f'AA{location_line_index}', total_received_qty, top_center_cell)
            sheet.write(f'AC{location_line_index}', total_good_received_qty, top_center_cell)
            sheet.write(f'AE{location_line_index}', total_scrap_qty, top_center_cell)
            sheet.write(f'AG{location_line_index}', total_repackaging_qty, top_center_cell)
            sheet.write(f'AI{location_line_index}', total_unpackaging_qty, top_center_cell)
            sheet.write(f'AK{location_line_index}', total_closing_qty, top_center_cell)
            row_index += 1
