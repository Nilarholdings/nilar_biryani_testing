...
class StockMultiScrap(models.Model):
    ...
    prepared_employee_id = fields.Many2one('hr.employee', string='Prepared Employee')
    prepared_employee_sign = fields.Binary(string='Prepared Sign')
    prepared_date = fields.Datetime('Prepared Date')
    verified_employee_id = fields.Many2one('hr.employee', string='Verified Employee')
    verified_employee_sign = fields.Binary(string='Verified Sign')
    verified_date = fields.Datetime('Verified Date')
    approved_employee_id = fields.Many2one('hr.employee', string='Approved Employee')
    approved_employee_sign = fields.Binary(string='Approved Sign')
    approved_date = fields.Datetime('Approved Date')

    @api.model
    def default_get(self, fields_list):
        vals = super(StockMultiScrap, self).default_get(fields_list)
        default_analytic_account_id = self.env.user.def_analytic_account_id.id
    ...
    def action_prepare(self):
        self.prepared_employee_sign = self.env.user.employee_id.user_signature
        self.prepared_employee_id = self.env.user.employee_id.id
        self.prepared_date = fields.datetime.now()
        return self.write({'state': 'prepare'})

    def action_verified(self):
        self.verified_employee_sign = self.env.user.employee_id.user_signature
        self.verified_employee_id = self.env.user.employee_id.id
        self.verified_date = fields.datetime.now()
        return self.write({'state': 'verified'})

    def action_validate(self):
        self.ensure_one()
        self.approved_employee_sign = self.env.user.employee_id.user_signature
        self.approved_employee_id = self.env.user.employee_id.id
        ...
        self.line_ids.move_id.move_line_ids.write({'date': self.excepted_date,'analytic_tag_ids':self.analytic_tag_ids})
        return self.write({'state': 'done'})
...
class MultiScrapLine(models.Model):
    ...
    analytic_tag_ids = fields.Many2many('account.analytic.tag',string="Analytic Tags",
                                        related='multi_scrap_id.analytic_tag_ids')
