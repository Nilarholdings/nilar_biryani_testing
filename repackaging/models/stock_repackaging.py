...
class StockRepackaging(models.Model):
    ...
    def _default_dest_location_id(self):
        for repack in self:
            location = repack.env['stock.location'].search([('is_repackaging', '=', True)])
            if len(location) > 1:
                raise UserError(_("Repackaging location must have  the one location."))
        return self.env['stock.location'].search([('is_repackaging', '=', True)], limit=1)
    ...
    def onchange_line_location(self):
        for rec in self:
            rec.stockrepackagingline_id.location_id = rec.location_id
    ...
    def default_get(self, fields_list):
        ...
        analytic_id = self.env.user.def_analytic_account_id.id
        if analytic_id:
        ...
        for rec in self:
            ...
            if not rec.onhand_qty:
                if rec.product_id:
                    ...
                    if stock_quant_ids:
                        rec.onhand_qty = sum(stock_quant_ids.mapped('quantity'))
    ...
    def copy(self, default=None):
        ...
        if self.product_id.child_id:
            ...
            if product_id:
                ...
                if stock_quant_ids:
                    onhand = sum(stock_quant_ids.mapped('quantity'))
