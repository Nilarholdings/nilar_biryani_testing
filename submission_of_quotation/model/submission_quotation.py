...
class SubmissionQuotation(models.Model):
    ...
    def action_submit(self):
        ...
        if self.env.user.employee_id:
            data = self.env.user.employee_id.id
        ...
        if not check_already_approved:
            ...
            for key, records in data.items():
                ...
                self.env['purchase.order'].create({
                    ...
                    'date_order': first_line.purchase_agreement_id.date_end,
                    'date_planned': first_line.purchase_agreement_id.date_end,

                })

            if self.env.user.employee_id:
                employee_id = self.env['hr.employee'].browse(int(self.env.user.employee_id))
            else:
