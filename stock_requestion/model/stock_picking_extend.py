...
class StockPicking(models.Model):
    ...
    requestion_id = fields.Many2one('stock.requestion', string='Requestion',copy=True)
    ...
    def button_validate(self):
        ...
        for pick in self:
            if pick.is_good_issued:
                pick.write({
                    'issued_by_sign': pick.env.user.employee_id.user_signature,
                    'issued_by_id': pick.env.user.employee_id.id
                })
            elif pick.is_good_received:
                pick.write({
                    'received_by_sign': pick.env.user.employee_id.user_signature,
                    'received_by_id': pick.env.user.employee_id.id
                })
            else:
                pick.write({
                    'issued_by_sign': pick.env.user.employee_id.user_signature,
