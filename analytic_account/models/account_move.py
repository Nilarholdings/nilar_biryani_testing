...
class AccountJournal(models.Model):
    ...
    def create(self, values):
        journal_id = super(AccountJournal, self).create(values)
        self.env.user.write({'journal_ids': [(4, journal_id.id)]})
        return journal_id
...
class AccountMove(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account',  default=lambda self: self.env.user.def_analytic_account_id)
    analytic_user = fields.Many2one('res.users')
    ...
    def default_get(self, fields_list):
        ...
        journal_id = self.env.user.default_journal_id.id
        analytic_account_id = self.env.user.def_analytic_account_id.id
        if move_type in ['in_invoice', 'in_refund', 'in_receipt']:
            if self.journal_id.type != 'purchase':
                allow_journals = self.env.user.journal_ids
    ...
    def onchange_journal(self):
        ...
        for rec in self:
            if rec.move_type == 'entry':
                default_journal_id = self.env.user.default_journal_id.id
                if not default_journal_id:
                    raise ValidationError(
                        _("Default Journal ID is Missing in User - ( %s)!" % (self.env.user.name)))
                else:
                    return {'domain': {'journal_id': [('id', 'in', self.env.user.journal_ids.ids)]}}
