...
from odoo.tools.misc import OrderedSet, format_date, groupby
...
class AccountMoveLine(models.Model):
    ...
    def _generate_price_difference_vals(self, layers):
        ...
        for layer in layers:
            ...
            unit_valuation_difference_curr = product_uom._compute_price(unit_valuation_difference_curr,
                                                                        self.product_uom_id)
            out_qty_to_invoice = product_uom._compute_quantity(out_qty_to_invoice, self.product_uom_id)
            if not float_is_zero(unit_valuation_difference_curr * out_qty_to_invoice,
            ...
            po_pu_curr = po_line.currency_id._convert(po_line.price_unit, self.currency_id, self.company_id,
                                                      self.move_id.invoice_date or self.date or fields.Date.context_today(
                                                          self), round=False)
            ...
            if not float_is_zero(unit_valuation_difference * qty_to_correct,
                ...
                svl_vals = self._prepare_pdiff_svl_vals(layer, sign * qty_to_correct, unit_valuation_difference,
    ...
    def _prepare_pdiff_svl_vals(self, corrected_layer, quantity, unit_cost, pdiff):
        ...
        common_svl_vals = {
            ...
            'remaining_qty': 0,
            'remaining_value': 0,
            'description': self.move_id.name and '%s - %s' % (
            self.move_id.name, self.product_id.name) or self.product_id.name,
        }
        return {
