...
class PurchaseOrder(models.Model):
    ...
    prepare_by_sign = fields.Binary(string='Prepare Sign', help='For Req By Sign',related='prepare_by_name.user_signature')
    ...
    def onchange_login_employee(self):
        for rec in self:
            analytic_ids = rec.env.user.allow_analytic_account_id.ids
            if analytic_ids:
...
class PurchaseOrderLine(models.Model):
    ...
    def write(self, values):
        for rec in self:
            old_uom = rec.multi_uom_line_id.uom_id.name
            old_purchase_uom_qty =rec.purchase_uom_qty
