...
class StockMove(models.Model):
        ...
        for move in self.filtered(lambda x: x.production_id or x.raw_material_production_id):
            ...
            if move.move_line_ids:
                move.move_line_ids.write({'production_id': move.raw_material_production_id.id,
                                          'date': move.production_id.date_planned_start or move.raw_material_production_id.date_planned_start,
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        for rec in res:
            if self.production_id:
                rec.update({
                    'date': self.production_id.date_planned_start.date()
                })
            if self.raw_material_production_id.date_planned_start:
                rec.update({
                    'date': self.raw_material_production_id.date_planned_start.date()
                })
            if self.unbuild_id:
                rec.update({
                    'date': self.unbuild_id.mo_id.date_planned_start.date()
                })
    ...
    def write(self, values):
        ...
        material_ids = []
        for rec in self:
            # changes to update from stock move to material
            mrp_ids = self.env['mrp.production'].search([('procurement_group_id', '=', rec.group_id.id)])
            if mrp_ids:
    ...
    def _get_price_unit(self):
        if self.production_id and not self.byproduct_id and not self.propagate_cancel:
    ...
    def _create_in_svl(self, forced_quantity=None):
        ...
        for move in self:
            ...
            if move.production_id and not move.byproduct_id and not move.propagate_cancel:
    ...
    def _generate_valuation_lines_data(self, partner_id, qty, debit_value, credit_value, debit_account_id,
        ...
        if self.unbuild_id and not self.byproduct_line_id and not self.raw_material_production_line_id:
            ...
            # Check for imbalance and adjust lines if necessary
            ...
            if imbalance != 0:
                adjustment_line = {
                    ...
                    'product_uom_id': self.product_id.uom_id.id,
                    'partner_id': partner_id,
                    'debit': -imbalance if imbalance < 0 else 0,
                    'credit': imbalance if imbalance > 0 else 0,
                    'account_id': debit_account_id if imbalance < 0 else credit_account_id,
                }
                lines.append(adjustment_line)

            # Build move lines dictionary
            move_lines = {index: line for index, line in enumerate(lines)}
        ...
        if self.production_id and not self.byproduct_id and not self.propagate_cancel:
            ...
            if self.location_id.usage == 'production' and self.propagate_cancel == False:
                lines.append({
                    'name': description,
                    'product_id': self.product_id.id,
                    'quantity': qty,
                ...
                for material_cost_line in production.pro_material_cost_ids:
                    product_price.append({
                        'product': material_cost_line.product_id.name,
                        'cost': round(material_cost_line.total_actual_cost, 2),
                    })
                    product = self.env['product.product'].search(
