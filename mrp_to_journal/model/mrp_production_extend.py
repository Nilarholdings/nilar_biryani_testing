...
class MrpProduction(models.Model):
    ...
    def write(self, values):
        ...
        for production in self:
            ...
            for raw in production.move_raw_ids:
                consume_qty = raw.bom_line_id.product_multi_uom_qty * production.planned_qty_avg  # etk
                raw.multi_uom_qty = consume_qty
    ...
    def _compute_total_cost(self):
        ...
        for line in self.pro_material_cost_ids:
            ...
            # changing line.cost to line.actual_cost for getting unblance issue by etk
            total_m_cost += round(line.actual_qty * line.cost, 2)
            material_total += round(line.total_actual_cost, 2)
            total_m_cost += round(line.actual_qty * line.cost, 2)
            material_actual_total += round(line.total_actual_cost, 2)
    ...
    def _compute_total_all_cost(self):
        ...
        actual_total = 0.0
        for mrp in self:
            total = mrp.total_labour_cost + mrp.total_overhead_cost + mrp.total_material_cost
            actual_total = mrp.total_actual_labour_cost + mrp.total_actual_overhead_cost + mrp.total_actual_material_cost
            mrp.total_all_cost = round(total, 2)
            mrp.total_actual_all_cost = round(actual_total, 2)

    def _compute_total_actual_qty(self):
        for mrp in self:
            mrp.total_actual_qty = mrp.qty_producing
    ...
    def _compute_planned_qty_avg(self):
        for production in self:
            if production.bom_id:
                production.planned_qty_avg = production.product_multi_uom_qty / production.bom_id.product_qty
            else:
    ...
    #Overwrite method for double click time finish goods stock move
    #adding [:1] at finish_moves
    def _post_inventory(self, cancel_backorder=False):
        ...
        for order in self:
            finish_moves = order.move_finished_ids.filtered(lambda m: m.product_id == order.product_id and m.state not in ('done', 'cancel'))[:1]
        ...
        self.finished_move_line_ids.write({'date': self.date_planned_start})
        self.move_finished_ids.write({'date': self.date_planned_start})
        self.move_raw_ids.write({'date': self.date_planned_start})
        self.move_dest_ids.write({'date': self.date_planned_start})
        self.move_byproduct_ids.write({'date': self.date_planned_start})
        return True

    def action_confirm(self):
        res = super().action_confirm()
        if self.picking_ids.filtered(lambda p: p.state not in ('cancel', 'done')):
            self.picking_ids.write({'scheduled_date': self.date_planned_start})
        return res