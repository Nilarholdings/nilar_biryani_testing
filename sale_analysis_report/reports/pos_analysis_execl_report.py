...
class PosAnalysisExcelReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, docs):
        ...
        y_offset += 1
        start_date = (datetime.strptime(date_from, '%Y-%m-%d %H:%M:%S') + relativedelta(hours=6, minutes=30)).strftime('%Y-%m-%d %H:%M:%S')
        sheet.merge_range(y_offset, 0, y_offset, 1, _('Start Date:'), format1)
        sheet.merge_range(y_offset, 2, y_offset, 3, start_date, format1)
        y_offset += 1

        end_date = (datetime.strptime(date_to, '%Y-%m-%d %H:%M:%S') + relativedelta(hours=6, minutes=30)).strftime(
            '%Y-%m-%d %H:%M:%S')
        ...
        product_totals = defaultdict(lambda: defaultdict(lambda: {'total_order_qty': 0, 'total_subtotal': 0}))

        for item in lines:
            sale_price_key = str(item['sale_price'])  # Convert sale_price to string
            product_totals[item['product_id']][sale_price_key]['total_order_qty'] += item['order_qty']
            product_totals[item['product_id']][sale_price_key]['total_subtotal'] += item['subtotal']

        pos_line_data = []

        for product, sale_prices in product_totals.items():
            product_id = self.env['product.product'].browse(product)
            for sale_price, totals in sale_prices.items():
                pos_line_data.append({
                    'product_id': product,
                    'sale_price': float(sale_price),  # Convert sale_price back to float if needed
        ...
        for category, records in grouped_records:
            ...
            for rec in records:
                ...
                total_sale_price += rec.get('sale_price')
                ...
                sheet.write('E{}'.format(row_index), rec.get('sale_price'), format3)
